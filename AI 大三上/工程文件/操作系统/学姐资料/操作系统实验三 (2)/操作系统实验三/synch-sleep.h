...
class Semaphore {
  ...
  private:
    char* name;        // useful for debugging,name of the lock
...
class Lock {
  ...
  private:
    char* name;				// for debugging,name of the lock
    List *lockqueue;            //用于存放阻塞的线程
    Thread *lockhoder;        //用于存放当前锁的拥有线程
    bool lockvalue;             //用于判断锁是否被锁上
