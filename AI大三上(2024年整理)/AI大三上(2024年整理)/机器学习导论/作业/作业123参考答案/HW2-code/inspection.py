...
class MajorityVoteClassifier:
    ...
    def fit(self, lbls):
        ...
        """
        self.cls = int(np.sum(lbls == 1) >= np.sum(lbls == 0))  # 如果1的个数≥0，则class就为1
...
def calc_error_from_file(pred_file, labels):
    ...
    """
    preds = read_preds(pred_file)# 把str转为np.ndarray格式
...
def calc_entropy(labels):
    ...
    """
    entropy     = 0
    unique_lbls = np.unique(labels)  # 获得labels为0的个数和labels为1的个数
    total_lbls  = len(labels)        # 总的labels的个数
...
def run_inspection(data, out_file):
    ...
    """
    lbls    = data[:, -1]   # 获取data的最后一列labels
    entropy = calc_entropy(lbls)   # 计算交叉熵
    ...
    error = calc_error(preds, lbls)   # 计算用majority_vote_classifier分类的error_rate
...
if __name__ == "__main__":
    ...
    data_interface.read_tsv(input)  # 读的数据放在了data中了
...
# python inspection.py heart_train.tsv heart_inspect.txt
