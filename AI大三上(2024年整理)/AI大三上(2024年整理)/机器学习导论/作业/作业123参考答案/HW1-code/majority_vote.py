...
class DataInterface:
    ...
    def get_data(self, keys = None):
        ...
        if keys is None:
            ...
            columns = np.zeros(self.data.shape[1], dtype=bool) # self.data.shape=(200,11)
            for key in keys:
...
class MajorityVoteClassifier:
    ...
    def fit(self, data):
        ...
        """
        labels   = data[:, -1]  # 取出最后一列的200个label
...
def write_labels(data, classifier, out_file):
    ...
    with open(out_file, "w+") as f:
        for entry in data:
            pred = classifier.infer(entry)   # classifier inference
            f.write(f"{pred}\n")
...
def read_preds(label_file):
    ...
    with open(label_file, "r") as f:
        lines = f.readlines()
        for line in lines:   # line is string like "0\n"
            preds.append(int(line[0]))   # line[0]=0
    return np.array(preds)   # 把所有样本的预测结果存入preds list中
...
def calc_error(preds, labels):
    ...
    Returns:
        float: percent error
    """
    return np.sum(np.abs(preds - labels)) / np.prod(np.size(labels))  # 不一样的结果就是错误的/整体的样本数
...
if __name__ == "__main__":
    ...
    train_data = interface.get_data()   # (200,11)

    # Train the classifier
    mv_cls = MajorityVoteClassifier()
    mv_cls.fit(train_data)     # 获得majority_vote_classifier的结果
    ...
    write_labels(train_data, mv_cls, train_out)   # 用majority_vote_classifier对train_data分类
    write_labels(test_data, mv_cls, test_out)     # 用majority_vote_classifier对test_data分类
