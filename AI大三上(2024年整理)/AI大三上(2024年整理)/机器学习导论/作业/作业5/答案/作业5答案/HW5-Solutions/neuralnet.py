...
To prevent potential type annotation issues, please run this script on Python 3.9 or later versions.
...
def random_init(shape):
    ...
    # ---
    # Initialize weights in the range [-0.1, 0.1] for columns excluding the first column (biases)
    W = np.random.uniform(-0.1, 0.1, (M, D-1))
    
    # Add a column of zeros at the beginning for biases
    biases = np.zeros((M, 1))
    W = np.hstack((biases, W))
    # ---
...
def cross_entropy(y: np.ndarray, y_hat: np.ndarray) -> np.ndarray:
    ...
    '''
    # ---
    # Fetch the predicted probabilities using y as indices
    predicted_probs = y_hat[y]
    
    # Calculate cross entropy
    loss = -np.log(predicted_probs)
...
def d_softmax_cross_entropy(y: np.ndarray, y_hat: np.ndarray) -> np.ndarray:
    ...
    '''
    # ---
    # Convert y into its one-hot representation
    y_one_hot = np.zeros_like(y_hat)
    y_one_hot[y] = 1
    
    # Compute the gradient
    gradient = y_hat - y_one_hot
...
class Sigmoid(object):
    ...
    def forward(self, x: np.ndarray) -> np.ndarray:
        ...
        # Store the sigmoid output in the cache for backward pass
        self.cache['sigmoid_output'] = sigmoid_output
    ...
    def backward(self, dz: np.ndarray) -> np.ndarray:
        ...
        '''
        # ---
        # Retrieve the cached sigmoid output
        sigmoid_output = self.cache['sigmoid_output']
        
        # Compute the gradient of the sigmoid
        dsigmoid = sigmoid_output * (1 - sigmoid_output)
...
class Linear(object):
    def __init__(self, input_size: int, output_size: int, 
        ...
        # shape you pass in.
        # ---
        # Initialize weight matrix for this layer with an additional column for bias
    ...
    def forward(self, x: np.ndarray) -> np.ndarray:
        ...
        :param x: Input to linear layer with shape (input_size,)
                    Zwhere input_size *does not include* the folded bias.
        ...
        '''
        # ---
        # Add bias to the input (folded bias)
        x_bias = np.concatenate([np.ones(1), x], axis=0)
        
        # Compute the output
        z = np.dot(self.w, x_bias[:, None])
        
        # Cache the input with bias for the backward pass
    ...
    def backward(self, dz: np.ndarray) -> np.ndarray:
        ...
        # the weight matrix!
        # ---
        # Retrieve the cached input with bias
        x_bias = self.cache['x_bias']
        
        # Compute the gradient with respect to weights
        self.dw = np.dot(dz, x_bias[None, :])

        # Compute the gradient with respect to input (excluding bias)
...
class NN(object):
    def __init__(self, input_size: int, hidden_size: int, output_size: int, 
        ...
        # ---
        # Initialize the linear layer from input to hidden layer
        ...
        # Initialize the linear layer from hidden layer to output
    ...
    def forward(self, x: np.ndarray) -> np.ndarray:
        ...
        # ---
        # First linear layer
        a = self.linear1.forward(np.squeeze(x))
        
        # Sigmoid activation layer
        z = self.activation.forward(np.squeeze(a))
        
        # Second linear layer
        b = self.linear2.forward(np.squeeze(z))
        
        # Softmax to get predictions
        y_hat = softmax(np.squeeze(b))
    ...
    def backward(self, y: np.ndarray, y_hat: np.ndarray) -> None:
        ...
        '''
        # ---
        # Compute gradient of the loss with respect to softmax input
        dz2 = d_softmax_cross_entropy(y, y_hat)
        
        # Backward pass from output to hidden layer
        da1 = self.linear2.backward(np.squeeze(dz2)[:, None])
        
        # Gradient through activation function
        dz1 = self.activation.backward(np.squeeze(da1))
        
        # Backward pass from hidden layer to input
        self.linear1.backward(np.squeeze(dz1)[:, None])
        # ---
    ...
    def step(self):
        ...
        # 由于 Sigmoid 激活函数没有权重，我们不需要为其调用 step 函数
    ...
    def print_weights(self, index) -> None:
        ...
        # np.savetxt(str(index) + '_W1.txt', self.linear1.w, delimiter=',')
        logging.debug(f"shape of w2: {self.linear2.w.shape}")
        logging.debug(self.linear2.w)
        # np.savetxt(str(index) + '_W2.txt', self.linear2.w, delimiter=',')
...
def test(X: np.ndarray, y: np.ndarray, nn: NN) -> tuple[np.ndarray, float]:
    ...
    for i in range(X.shape[0]):
        ...
        # 如果预测标签不等于真实标签，增加错误计数器
        if predicted_label != y[i]:
            incorrect_count += 1

    # 计算错误率
    error_rate = incorrect_count / X.shape[0]

    return np.array(predicted_labels), error_rate
    # ---
...
def train(X_tr: np.ndarray, y_tr: np.ndarray, 
    ...
    for epoch in range(n_epochs):
        ...
        for x, y_true in zip(X_shuffled, y_shuffled):
            y_hat = nn.forward(x)
            nn.backward(y_true, y_hat)
            nn.step()
            
            # 在 epoch 的前3次更新时，打印权重
            if counter < 4:
                nn.print_weights(counter)
                counter += 1
            
        # After each epoch, we compute the loss on both train and test sets
        train_loss = np.mean([cross_entropy(y, nn.forward(x)) for x, y in zip(X_tr, y_tr)])
        test_loss = np.mean([cross_entropy(y, nn.forward(x)) for x, y in zip(X_test, y_test)])
...
if __name__ == "__main__":
    ...
    # ---
    nn = NN(input_size=X_tr.shape[1], 
        hidden_size=n_hid, 
        output_size=len(labels), 
        weight_init_fn=random_init if init_flag == 1 else zero_init, 
        learning_rate=lr)
    # ---
