...
Interrupt::SetLevel(IntStatus now)
{
    ...
    ChangeLevel(old, now);			// 把状态从old改成now
    if ((now == IntOn) && (old == IntOff))// 现在的开了，以前的关了
...
// check any pending interrupts are now ready to fire
	...
	;
    ChangeLevel(IntOff, IntOn);		// 中断再次开启
...
Interrupt::CheckIfDue(bool advanceClock)
{
	...
	stats->idleTicks += (when - stats->totalTicks);// when - stats->totalTicks这么长时间啥也没干
	stats->totalTicks = when;// 快进
...
// Check if there is nothing more to do, and if so, quit
    ...
    (*(toOccur->handler))(toOccur->arg);	// 线程叫醒服务
