...
// Implement lock and condition variable via semaphore.
...
class Lock {
  ...
  private:
    ...
    Semaphore *sem;         // semaphore for lock
    Thread *current;        // note down the current thread
};
...
class Condition {
  ...
  private:
    char* name;
    // plus some other stuff you'll need to define
    Semaphore *s, *x, *h;   // use names used in Implementing Condition
                            // Variables with Semaphores, A. D. Birrell, 2003
    int waiters;
};
#endif // SYNCH_SEM_H
