...
extern bool allowSynch;
extern int producerSize;
extern int consumerSize;
#endif
...
main(int argc, char **argv) {
    ...
    //     -e [type_of_error] -a [allow_synchronous_primitives(0 for FALSE/1 for TRUE)]

    // usage for lab 2.3 using lock & condition variable:
    //   ./nachos -q 5 -t [num_threads] -n [num_items] -s [table_size] 
    //     -e [type_of_error] -a [allow_synchronous_primitives(0 for FALSE/1 for TRUE)]
    // -p [producer write size] -c[consumer read size]

    // usage for lab 2.3 using semaphore:
    ...
    break;
          case 'p':
              producerSize = atoi(argv[1]);
              argCount++;
              break;
          case 'c':
              consumerSize = atoi(argv[1]);
              argCount++;
              break;
      default:
        testnum = 1;
